...
describe('SearchSidebar Component', () => {
    ...
    beforeEach(() => {
        ...
        // Setup mock store responses using guild-scoped search state
        (useAppStore as any).mockImplementation((selector: any) => {
            const state = {
                searchStateByGuild: {
                    'sv1': { isOpen: true, query: 'disease', results: mockMessages }
                },
                setSearchSidebarOpen: mockSetSearchSidebarOpen,
                setSearchQuery: mockSetSearchQuery,
                setSearchResults: mockSetSearchResults,
                guildMap: { 'sv1': 'http://localhost:3001' },
    ...
    it('renders empty state when no results and not searching', () => {
        // Override mock for this test — guild has search open with no results
        (useAppStore as any).mockImplementation((selector: any) => {
            const state = {
                searchStateByGuild: {
                    'sv1': { isOpen: true, query: 'notfound', results: [] }
                },
                setSearchSidebarOpen: mockSetSearchSidebarOpen,
                setSearchQuery: mockSetSearchQuery,
                setSearchResults: mockSetSearchResults,
